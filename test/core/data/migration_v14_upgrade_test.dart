...
void main() {
  ...
  const String orderId = 'ord-v13-legacy-1';
  ...
  setUp(() async {
    directory = await Directory.systemTemp.createTemp('brisko_v14_upgrade');
    path = p.join(directory.path, 'upgrade.db');
    await _createVersion13Database(path, orderId: orderId, billedAt: billedAt);
  });
  ...
  test('a v13 database reaches the current schema version', () async {
  ...
  test('the customerName column is added to the orders table', () async {
    ...
    final Set<String> names = columns
        ...
        .toSet();
    expect(names, contains('customerName'));
  });

  test('a bill settled before the column reads back with no name', () async {
    final SqliteDatabase database = await TestDatabase.openOnDisk(path);
    addTearDown(database.close);
...
Future<void> _createVersion13Database(
  String path, {
